...
unpack ( execute( Cl ([ ( Var("z") , Cl ( [] , C 3 )) ], V(Var("z")) ) , [] ) ) ;;
unpack ( execute( Cl ( [] , ADD ( ADD ( C 2 , C 3 ), ADD ( C 2 , C 3 ))), [] ) ) ;;
unpack ( execute( Cl ([ ( Var("z") , Cl ( [] , C 3 )) ], ADD ( C 2 , V(Var("z")) ) ) , [] ) ) ;;
unpack ( execute( Cl ( [] , Apply ( Lambda ( Var("x") ,  ADD ( V(Var("x")) , C 1 )), C 2 )), [] ) ) ;;
unpack ( execute( Cl ( [] , Apply ( Lambda ( Var("x") ,  MUL ( V(Var("x")) , ADD ( V(Var("x")) , C 1 ))), C 2 )), [] ) ) ;;
unpack ( execute( Cl ( [] , Apply ( Lambda ( Var("x") ,  Apply ( Lambda ( Var("d") ,  MUL ( V(Var("d")) , C 2 )), C 2 )), C 2 )), [] ) ) ;;
unpack ( execute( Cl ( [] , Ifte ( GT ( C 8 , C 2 ), Apply ( Lambda ( Var("x") , DIV ( V(Var("x")) , C 2 )), C 2 ), Apply ( Lambda ( Var("x") ,  MUL ( V(Var("x")) , ADD ( V(Var("x")) , C 1 ))), C 2 ))), [] ) ) ;;
unpack ( execute( Cl ( [] , Ifte ( GT ( C 1 , C 2 ), Apply ( Lambda ( Var("x") , DIV ( V(Var("x")) , C 2 )), C 2 ), Apply ( Lambda ( Var("x") ,  MUL ( V(Var("x")) , ADD ( V(Var("x")) , C 1 ))), C 2 ))), [] ) ) ;;
unpack ( execute( Cl ( [] , Let ( Var("a") , C 2 , ADD ( V(Var("a")) , C 20 ) ) ), [] ) ) ;;
unpack ( execute( Cl ( [] , Let ( Var("a") , C 2 , ADD ( V(Var("a")) , C 20 ) ) ), [] ) ) ;;
unpack ( execute( Cl ( [] , Proj ( 2 , Tuple( 3, [ C 1 ; C 2 ; C 3 ] ) ) ), [] ) ) ;;(* Check 2 or 1 in above projection according to index (0 indexed) or kth position(1 indexed) *)
unpack ( execute( Cl ( [] , Apply ( Lambda ( Var("x") ,  Let ( Var("a") , C 2 , ADD ( V(Var("a")) , V(Var("x")) ))) , C 2 )) , [] ) ) ;;


(* Made the Def , Not, Or, And, Xor, Implies, Mod, Sub, Abs, Pow, Max-Min, Equal, GTE, LT, LTE and other helper expressions seperately. *)
